 ...
 * then mounting `express.json()` + the auth, admin and workspace routers. Equivalent of the
 * reference combo's `AuthModule.forRoot(...)`, just a function instead of a NestJS dynamic
 * module — there's no DI container here, so wiring is explicit constructor calls in one place.
 */
export function createAuthApp(config: Partial<AuthConfig> = {}): Express {
  ...
  app.use("/auth/admin", createAdminRouter({ auth: authService, authentication: requireAuth, workspaceScope: requireWorkspace }));
  ...
  app.use(
    "/workspaces",
    createWorkspaceRouter({ workspaces, rbac, authentication: requireAuth, authorization: requireAuthz, workspaceScope: requireWorkspace }),
  );
