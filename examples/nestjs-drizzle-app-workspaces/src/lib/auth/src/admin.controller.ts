 ...
 * name: `WorkspaceGuard` resolves the caller's membership of the workspace named by the
 * `X-Workspace-Id` header into the roles and permissions that membership carries *there*, builds
 * the CASL ability from them, and `AbilityGuard` checks each route's `@CheckAbility` against it —
 ...
 *
 * Two consequences worth being explicit about. An admin of workspace A calling with workspace B's
 * id is rejected by the guard, because the context it resolves is B's membership — the check is
 * workspace-scoped by construction, not by a second check. And there is no role-based bypass:
