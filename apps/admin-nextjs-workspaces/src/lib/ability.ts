...
/**
 * The one place the console decides what the signed-in user may do *in the active workspace*.
 * Every screen asks this module and nothing else — no page reads `currentUser.permissions` (or
 * `roles`) directly — so there is a single seam to move when the backend changes how it reports
 * authority.
 *
 * Authority here is per-workspace: `/auth/me` answers with the roles and permissions of the
 * membership named by `X-Workspace-Id`, so switching workspaces re-reads it (see
 * `WorkspaceStore`) and this ability is rebuilt from the new answer. The same user can be an
 * administrator in one workspace and an ordinary member in the next.
 *
 ...
 * workspace members is a capability only this variant has.
 */
 ...
 * only — a role called "admin" that carries nothing in this workspace gets the same 403 as no
 * role at all — so a wildcard for `roles.includes("admin")` would make the console promise what
 * the server refuses.
 */
export function buildAbility(user: CurrentUser | null): AppAbility {
...
export function missingPermissionHint(permissionKey: PermissionKey): string {
  return `You need the "${permissionKey}" permission in this workspace to do this.`;
}
