          ...
          return (
            <Link to={`/users/${userIdOf(user)}`} className="flex items-center gap-2 hover:underline">
              ...
              <div className="flex flex-col">
                ...
                {user.displayName ? <span className="text-xs text-muted-foreground">{user.email}</span> : null}
              </div>
            </Link>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            {row.original.roles.length === 0 ? <span className="text-xs text-muted-foreground">—</span> : null}
