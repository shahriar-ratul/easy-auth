...
//
// Each variant is materialized, its Prisma client generated, its migrations applied to its own
// database, seeded, and then `test/prove-cycle.ts` is run inside it with AUTH_VARIANT set — the
// test asserts the shared auth cycle for both, plus the workspace isolation properties for the
// workspace variant.
//
...
// which is what previously let the suite pass locally and fail on a genuinely fresh database.
//
